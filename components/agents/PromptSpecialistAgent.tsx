...
const PromptSpecialistAgent: React.FC<AgentProps> = ({ onBack, history, addToHistory, deleteHistoryItem, clearAgentHistory }) => {
    ...
    const handleGeneratePrompt = async () => {
        ...
        const systemInstruction = "Você é um especialista em engenharia de prompts para modelos de IA generativa de imagem e vídeo. Sua tarefa é pegar uma ideia simples do usuário e transformá-la em um prompt em inglês, extremamente detalhado e eficaz, otimizado para o modelo alvo.";
        const userPrompt = `
            Baseado na seguinte ideia simples: "${idea}", crie um prompt detalhado e otimizado para um modelo de IA que gera ${targetModel === 'image' ? 'imagens' : 'vídeos'}.
        ...
        <div className="animate-fade-in">
            ...
            <div className="flex gap-8 mt-4" style={{ height: 'calc(100vh - 12rem)' }}>
                ...
                <div className="flex-grow space-y-6 overflow-y-auto pr-4">
                    <div>
                        <label htmlFor="idea" className="block text-sm font-medium text-zinc-300 mb-2">1. Descreva sua ideia</label>
                        <textarea
                            id="idea"
                            value={idea}
                            ...
                            rows={3}
                            className="w-full bg-black border border-zinc-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-white transition resize-none"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-zinc-300 mb-2">2. Qual o seu objetivo?</label>
                        <div className="grid grid-cols-2 gap-4">
                            <button 
                                onClick={() => setTargetModel('image')}
                                className={`px-4 py-3 text-sm font-semibold rounded-lg border-2 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:ring-white ${targetModel === 'image' ? 'bg-white border-white text-black' : 'bg-black border-zinc-800 text-zinc-400 hover:border-zinc-700'}`}
                            >
                                Gerar Imagem
                            </button>
                            <button 
                                onClick={() => setTargetModel('video')}
                                className={`px-4 py-3 text-sm font-semibold rounded-lg border-2 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:ring-white ${targetModel === 'video' ? 'bg-white border-white text-black' : 'bg-black border-zinc-800 text-zinc-400 hover:border-zinc-700'}`}
                            >
                                Gerar Vídeo
                            </button>
                        </div>
                    ...
                    {(generatedPrompt || isLoading) && (
                        <div className="space-y-2">
                             <label className="block text-sm font-medium text-zinc-300">3. Seu prompt otimizado</label>
                                ...
                                {generatedPrompt && (
                                    <>
                                        ...
                                        <button
                                            ...
                                            className={`absolute top-3 right-3 px-3 py-1 text-xs font-semibold rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:ring-white ${copySuccess ? 'bg-green-700 text-white' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}
                                        >
