...
import { AgentIcon, ContentIcon, CommunityIcon, MenuIcon, UserIcon, SettingsIcon, AdminIcon, LogoutIcon } from './icons';
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isCollapsed, setIsCollapsed, user, setUser, onLogout }) => {
  ...
  const NavLink: React.FC<{ item: typeof navItems[0] }> = ({ item }) => {
    ...
    return (
      <button
        ...
        className={`group relative flex items-center w-full px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-white ${
          isActive
  ...
  return (
    <>
      <div className={`relative flex flex-col bg-zinc-900 text-zinc-200 border-r border-zinc-800 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        <div className={`flex items-center h-16 shrink-0 ${isCollapsed ? 'justify-center' : 'px-4'}`}>
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-2 rounded-md hover:bg-zinc-800/60 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-white">
        ...
        <div className={`px-4 py-4 mt-auto border-t border-zinc-800`}>
           {isProfileMenuOpen && (
              <div 
                ref={profileMenuRef}
                className={`absolute bottom-20 ${isCollapsed ? 'left-20' : 'left-4'} z-20 w-60 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl animate-fade-in-up-fast`}>
                  <div className="p-2">
                      {profileMenuItems.map(item => (
                          <button key={item.label} onClick={item.action} className="w-full flex items-center px-3 py-2.5 text-sm text-zinc-200 rounded-lg hover:bg-zinc-800 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
                              <item.icon className="w-5 h-5 mr-4 text-zinc-400" />
           ...
           <button
              ...
              className={`group relative flex items-center w-full p-2 text-sm font-medium text-left text-zinc-300 rounded-lg hover:bg-zinc-800/60 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-white ${isCollapsed ? 'justify-center' : ''}`}
           >
